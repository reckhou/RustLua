...
function PLUGIN:cmdBan(netuser, cmd, args)
  ...
  if (b) then
    ...
    local allUsers = rust.GetAllNetUsers()
    for key, value in pairs(allUsers) do
      local target = value
      local curSteamID = self:FromNetUserToSteam64(target)
      if (curSteamID == args[1]) then
        target:Kick(NetError.Facepunch_Kick_Ban, true)
        break
      end
      print("curSteamID:", curSteamID, "arg[1]", args[1])
    end
  end
end
...
function PLUGIN:FromNetUserToSteam64(netUser)
	local userID = tonumber( rust.GetUserID( netUser ))
	local steamID = rust.CommunityIDToSteamID( userID )
  print("userID:", userID, "steamID:", steamID, "64:", self:ToSteam64(steamID))
	return self:ToSteam64( steamID )
end
